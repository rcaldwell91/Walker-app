...
import { AddLineForm, LineRow, PaymentForm } from "./invoice-forms";

const one = <T,>(x: T | T[] | null | undefined) => (Array.isArray(x) ? x[0] : x) ?? null;

export default async function WalkerInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: inv } = await supabase
    .from("invoices")
    .select(
      "id, number, status, client_id, period_start, period_end, issued_on, due_on, sent_at, client:clients(name), invoice_lines(id, kind, description, occurred_on, quantity, unit_cents, amount_cents, walk_id), payments(id, amount_cents, method, received_on, note)",
    )
    .eq("id", id)
